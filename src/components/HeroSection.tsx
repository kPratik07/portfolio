...
const HeroSection: React.FC = () => {
  const developerObject = `const developer = {
  name: "Pratik Raj",
  role: "Software Engineer",
  experience: "2 years",
  studentsMentored: "150+",
  passion: "Building meaningful products",
  focus: ["Clean Code", "Scalability", "Great UX"],
  alwaysLearning: true
};`;
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      index += 1;
      const nextCode = developerObject.slice(0, index);
      setTypedCode(nextCode);

      if (index >= developerObject.length) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setActiveEmojiIndex((prev) => (prev + 1) % emojiSequence.length);
    }, 1400);

    return () => clearInterval(interval);
  }, [emojiSequence.length, showEmojis]);

  const renderTypedCode = () => {
    if (!typedCode) return null;

    const keywordPattern =
      /(const|name|role|experience|studentsMentored|passion|focus|alwaysLearning)/g;
    const stringPattern = /("(?:[^"\\]|\\.)*"|true|false|null)/g;
    const parts: React.ReactNode[] = [];
    let lastIndex = 0;
    const combinedPattern = new RegExp(
      `${keywordPattern.source}|${stringPattern.source}`,
      "g",
    );

    let match: RegExpExecArray | null;
    while ((match = combinedPattern.exec(typedCode)) !== null) {
      const matchedValue = match[0];

      if (match.index > lastIndex) {
        parts.push(typedCode.slice(lastIndex, match.index));
      }

      const isKeyword = keywordPattern.test(matchedValue);
      parts.push(
        <span
          key={`${matchedValue}-${match.index}`}
          className={isKeyword ? "keyword" : "string"}
        >
          {matchedValue}
        </span>,
      );
    ...
    if (lastIndex < typedCode.length) {
      parts.push(typedCode.slice(lastIndex));
    }
  ...
  return (
    <>
      <style>{`
        ...
        .typed-code .keyword { color: #7c3aed; }
        .typed-code .string { color: #16a34a; }
        ...
        .emoji-bubble {
          ...
          animation: emoji-pop 0.55s ease-out;
      ...
      >
        ...
        <div className="relative z-10 mx-auto grid w-full max-w-[1000px] gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:gap-7">
          <div className="max-w-[420px] self-center pt-2">
            <div className="mb-7 inline-flex items-center gap-2 rounded-full border border-violet-100 bg-violet-50/70 px-3 py-1.5 text-s font-semibold text-violet-600">
          ...
          <div className="relative mx-auto w-full max-w-[600px] self-center lg:translate-x-2">
            <div className="relative overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_18px_35px_rgba(91,75,160,0.16)]">
              ...
              <div className="min-h-[260px] overflow-hidden bg-white p-5 font-mono text-[10px] leading-[1.9] text-slate-600 sm:min-h-[305px] sm:p-7 sm:text-[12px]">
                <pre className="typed-code">{renderTypedCode()}</pre>
                {showFinalComment && (
                  <div className="mt-3 flex items-center gap-2 text-slate-400">
                    <span>{"// Code. Solve. Build. Repeat."}</span>
                    {showEmojis && (
                      <span key={activeEmojiIndex} className="emoji-bubble">
                        {emojiSequence[activeEmojiIndex]}
                      </span>
                    )}
                  </div>
                )}
